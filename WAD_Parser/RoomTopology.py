...
def create_graph(floormap, return_dist=False, room_coordinates=False):
    ...
    for i in range(int(dist.max()), int(dist.min())-1,-1):
        local_max_coords = peak_local_max(dist, threshold_abs=threshold-i, labels=floormap, min_distance=3)
    ...
    local_max_coords = peak_local_max(dist, min_distance=3, labels=floormap, threshold_abs=optimal_threshold)
    local_max = np.zeros_like(dist, dtype=bool)
    local_max[tuple(local_max_coords.T)] = True
    markers = ndi.label(local_max)[0]
    roommap = watershed(-dist, markers, mask=floormap)

    if len(roommap.shape) == 3 and roommap.shape[2] == 3:  # Check if it's a color image with 3 channels
        grayscale_roommap = color.rgb2gray(roommap)
    else:
        grayscale_roommap = roommap  # If it's already grayscale, use it directly

    room_RAG_boundaries = skg.rag_boundary(roommap, filters.sobel(grayscale_roommap))
    if room_coordinates:
        # For each floor...
...
def topological_features(floormap, prepare_for_doom=False):
    ...
    for r in range(1, roommap.max() + 1):
        # Room Size
        room_graph.nodes[r]["area"] = room_props[r - 1]["area"]
        room_graph.nodes[r]["perimeter"] = room_props[r - 1]["perimeter"]
        mask = (roommap == r)
        max_dist = np.max(mask * dist)
        room_graph.nodes[r]["max_dist"] = max_dist
        room_graph.nodes[r]["centroid"] = room_props[r - 1]["centroid"]
    ...
    diam_list = list()
    assort_list = list()
    for component in nx.connected_components(graph_no_background):
        cc = graph_no_background.subgraph(component)
        if len(cc.edges()) > 0:
